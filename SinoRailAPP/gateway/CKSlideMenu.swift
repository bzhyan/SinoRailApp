...
class CKSlideMenu: UIView {
    ...
    private func setupTabScrollView()  {
        ...
        for (index ,title) in titlesArr.enumerated() {
            ...
            //计算title长度
            let size = (title as NSString).size(withAttributes: [kCTFontAttributeName as NSAttributedStringKey:font])
    ...
    private func updateFonts() {
        ...
        for item in itemLabels {
            //计算title长度
            let size = (item.text! as NSString).size(withAttributes: [kCTFontAttributeName as NSAttributedStringKey:font])
            item.frame = CGRect(x: originX, y: 0, width: size.width, height: self.frame.height)
            //设置属性
            item.font = font
    ...
    // 导航栏点击事件
    @objc func itemClicked(gesture:UITapGestureRecognizer) {
